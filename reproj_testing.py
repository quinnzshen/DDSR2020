...
print("relpose", rel_pose)
poset = get_pose(scene_path, target)
poses = get_pose(scene_path, source)
abs_pose = torch.eye(4)
abs_pose[:3, :3] = poset[:3, :3].t() @ poses[:3, :3]
abs_pose[:3, 3] = (poses[:3, 3] - poset[:3, 3])
abs_pose = abs_pose.numpy()

# abs_pose = (poset.inverse() @ poses).numpy()
# T @ S.I <no work>
# T.I @ S <WORKS>
# S.I @ T <no work>
# S @ T.I <kinda works>
...
source_coords = (world_coords @ the_pose.T)[:, :3] @ tgt_intrinsic.T
