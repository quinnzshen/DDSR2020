...
class Trainer:
    ...
    def run_epoch(self):
        ...
        print("Training epoch {}".format(self.epoch + 1), end=", ")
        ...
        print("Time spent: {}".format(train_end_time - train_start_time))
    ...
    def process_batch(self, batch_idx, batch, img_num, dataset_length, train):
        ...
        inputs = F.interpolate(batch["stereo_left_image"].to(self.device).permute(0, 3, 1, 2).float(), (self.height, self.width), mode="bilinear", align_corners=False)
        ...
        sources_list = [F.interpolate(batch["stereo_right_image"].to(self.device).permute(0, 3, 1, 2).float(), (self.height, self.width), mode="bilinear", align_corners=False)]
        ...
        for i in range(-self.prev_frames, self.next_frames + 1):
            ...
            sources_list.append(F.interpolate(batch["nearby_frames"][i]["camera_data"]["stereo_left_image"].to(self.device).permute(0, 3, 1, 2).float(), (self.height, self.width), mode="bilinear", align_corners=False))
            poses_list.append(
    ...
    def add_disparity_map_to_tensorboard(self, disp, img_num, dataset_length, train):
        ...
        self.writer.add_image(train_or_val + " - " + 'Epoch: {}, '.format(self.epoch + 1) + 'Image: {}'.format(img_num), im,
