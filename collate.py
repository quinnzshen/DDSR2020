...
def default_collate(batch, height=384, width=1280):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ == 'ndarray':
            ...
            return default_collate([torch.tensor(b, dtype=torch.float32) for b in batch])
        ...
        if 'stereo_right_image' in elem.keys():
            ...
            for key in resize_list:
                for d in batch:
                    d[key] = F.interpolate((torch.tensor(d[key].transpose(2, 0, 1), dtype=torch.float32).unsqueeze(0)),
                                                  [height, width], mode="bilinear", align_corners=False).squeeze(0)
